...
{  
    ...
    if (PRINT_DEBUG_CH) 
    {
		...
		printf("\n\t DAILY_COUNTER_TOTAL_DIVIDEND=%f STOCK_INDEX.weight=%f STOCK_INDEX.nr_shares=%d\n", DAILY_COUNTER_TOTAL_DIVIDEND, STOCK_INDEX.weight, STOCK_INDEX.nr_shares);
    }
    #endif
    ...
    if (STOCK_INDEX.nr_shares>0)
		STOCK_INDEX.dividend_per_share = DAILY_COUNTER_TOTAL_DIVIDEND /STOCK_INDEX.nr_shares;
...
{   
	DAILY_COUNTER_TOTAL_DIVIDEND=0.0;
    
    //Read dividend_info messages and add to counter
    START_DIVIDEND_INFO_MESSAGE_LOOP
		DAILY_COUNTER_TOTAL_DIVIDEND += dividend_info_message->total_dividend;	
    FINISH_DIVIDEND_INFO_MESSAGE_LOOP

    //Update stock index dividend
    STOCK_INDEX.total_dividend = DAILY_COUNTER_TOTAL_DIVIDEND;

    #ifdef _DEBUG_MODE
    if (PRINT_DEBUG_CH) 
    {
		printf("\n\n ClearingHouse_receive_dividend_info:");
		printf("\n\t DAILY_COUNTER_TOTAL_DIVIDEND=%f\n\n", DAILY_COUNTER_TOTAL_DIVIDEND);
    }
    #endif
...
{  
	...
	{
		#ifdef _DEBUG_MODE    
		if (PRINT_DEBUG_AFM_CH || PRINT_DEBUG)
		{
			 printf("Error in ClearingHouse_update_price(): STOCK_INDEX.price_history.size=%d\n", STOCK_INDEX.price_history.size);
		}    
		#endif
	}   
