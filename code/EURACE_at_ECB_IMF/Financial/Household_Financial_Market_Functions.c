...
int Household_receive_index_info()
{
    ...
    START_INDEX_INFO_MESSAGE_LOOP
        ...
        log_inflow("household", "payment_account", "net_worth", "dividends", ID, dividend);
    ...
    //CUM_TOTAL_DIVIDENDS: This is the cumulative dividends across the household-subjective month (reset on day_of_month_receive_income).
    //Because dividends are paid daily and taxes monthly, the household has to cumulate dividends along its subjective month, to correctly account for a monthly period of dividends.
    CUM_TOTAL_DIVIDENDS += dividend; 
    PAYMENT_ACCOUNT += dividend;  
    HOUSEHOLD_INFLOWS_CALENDAR.total_dividends += dividend;        

    //Sanity Check:
    enforce_hh_valid_payment_account("Household_receive_index_info");
...
int Household_update_portfolio()
{
	...
	else
	{
        ...
        if (order_status_message->value>0.0)
            log_outflow("household", "payment_account", "net_worth", "stock_market_transaction", ID, order_status_message->value);
        else
            log_inflow("household", "payment_account", "net_worth", "stock_market_transaction", ID, -order_status_message->value);
    ...
    //Sanity Check:
    enforce_hh_valid_payment_account("Household_update_portfolio");
