...
int Firm_calc_input_demands_2()
{
	...
	DEMAND_CAPITAL_STOCK = max(NEEDED_CAPITAL_STOCK-TOTAL_UNITS_CAPITAL_STOCK,0.0);
        ...
        /*This computes the financial needs for production*/
...
int Firm_calc_production_quantity()
{
	...
	/*Computing the current market share estimation as linear combination of the estimation of the beginning and the end of the year*/
		...
		for(j=0;j<CURRENT_MALL_STOCKS.size;j++)
		{
			if(CURRENT_MALL_STOCKS.array[j].mall_id==LINEAR_REGRESSION_MARKET_SIZE.array[i].mall_id)
			{
				...
				(LINEAR_REGRESSION_MARKET_SIZE.array[0].intercept 
					...
					/*Compute for each mall the critical stock*/
				...
				if (PRINT_DEBUG_PRODUCTION)
				{
					if(ID==ID_DEBUG_PROBE)
					{	
						printf("\n CURRENT_MALL_STOCKS.array[j].critical_stock %f LINEAR_REGRESSION_MARKET_SIZE.array[i].intercept %f LINEAR_REGRESSION_MARKET_SIZE.array[i].regressor %f COUNTER_MONTH_SINCE_LAST_PRICE_SETTING %d LINEAR_REGRESSION_MARKET_SIZE.array[i].variance %f Buffer: %f ESTIMATED_MARKET_SHARE %f \n", CURRENT_MALL_STOCKS.array[j].critical_stock, LINEAR_REGRESSION_MARKET_SIZE.array[i].intercept, LINEAR_REGRESSION_MARKET_SIZE.array[i].regressor,  COUNTER_MONTH_SINCE_LAST_PRICE_SETTING, LINEAR_REGRESSION_MARKET_SIZE.array[i].variance, local_buffer, ESTIMATED_MARKET_SHARE);
					}        
				}
 				 #endif		
...
{
	//fprintf(stderr,"\nIT %d ID %3d %s(), line 1", DAY, ID, __FUNCTION__);
	...
	reset_adt_technology_vintages_array(&TECHNOLOGY_VINTAGES);
	...
	{ 
		if(ID==ID_DEBUG_PROBE)
		{
			printf("\n\n IT %d ID: %d Firm_calc_input_demands ", DAY, ID);
			...
			printf("\n\t EMPLOYEES_NEEDED_INNOVATION=%d employees_needed_inno=%f  PLANNED_INNOVATION_EXPENDITURES=%f", EMPLOYEES_NEEDED_INNOVATION,employees_needed_inno, PLANNED_INNOVATION_EXPENDITURES);
			printf("\n\t PLANNED_EXPENDITURES_PRODUCTION_INNOVATION=%f \n", PLANNED_EXPENDITURES_PRODUCTION_INNOVATION);
...
int Firm_calc_production_quantity_2()
{
	...
	// Calculate remaining budget for hiring
	...
	if(REMAINING_BUDGET_FOR_HIRING < -EPSILON)
		REMAINING_BUDGET_FOR_HIRING = 0;

	assert(REMAINING_BUDGET_FOR_HIRING > -EPSILON);
...
int Firm_send_capital_demand()
{
	...
	//Simple fix for negative payment accounts (NEEDS TO BE REVISITED)
	DEMAND_CAPITAL_STOCK = min(DEMAND_CAPITAL_STOCK, max(0,NO_EMPLOYEES_PRODUCTION-TOTAL_UNITS_CAPITAL_STOCK));
...
int Firm_receive_capital_goods()
{
	...
	if(POLICY_EXP_BEST_TECHNOLOGY_SUBSIDY==1 && abs_double(TECHNOLOGY_VINTAGES.array[VINTAGE_SELECTED].productivity - TECHNOLOGICAL_FRONTIER)<1e-5)
	{
		CAPITAL_COSTS += capital_good_delivery_message->capital_good_delivery_volume * TECHNOLOGY_VINTAGES.array[VINTAGE_SELECTED].price;
...
int Firm_execute_production()
{
	...
	if(PLANNED_PRODUCTION_QUANTITY != 0 || NO_EMPLOYEES_PRODUCTION > 0)
	{     
...
int Firm_calc_pay_costs()
{
	...
	log_outflow("firm", "payment_account", "net_worth", "wages", ID, LABOUR_COSTS);
	log_outflow("firm", "payment_account", "net_worth", "capital_costs", ID, CAPITAL_COSTS);
	log_inflow("firm", "payment_account", "net_worth", "subsidies", ID, SUBSIDY_PAYMENT);


	assert__(PAYMENT_ACCOUNT > -1e-4)
	{
		fprintf(stderr, "\nIT %d ID %d %s(), line %d: WARNING: After paying PRODUCTION_COSTS %f: PAYMENT_ACCOUNT %f", DAY, ID, __FUNCTION__, __LINE__, PRODUCTION_COSTS, PAYMENT_ACCOUNT);
	}
    ...
    /*calc the daily revenue and sum up the monthly revenue*/
		...
		for(i=0; i< SOLD_QUANTITIES.size; i++)
		{
			if(sales_message->mall_id ==  SOLD_QUANTITIES.array[i].mall_id)
			{
				...
				{ 
					if ( (sales_message->revenue) <0.0)
					{
						printf("\n\n Firm_calc_revenue ID: %d",ID);
						printf("\n\t mall reported revenue: %f", (sales_message->revenue) );
					}
				}
				#endif
		...
		log_inflow("firm", "payment_account", "net_worth", "revenue", ID, REVENUE_PER_DAY);
