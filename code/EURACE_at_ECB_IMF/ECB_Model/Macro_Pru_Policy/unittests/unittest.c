...
int main(int argc, char * argv[])
{
    ...
    if(
        CU_add_test(pSuite, "policy_response 1", unittest_policy_response_1)  == NULL ||
        CU_add_test(pSuite, "policy_response 2", unittest_policy_response_2)  == NULL ||
        CU_add_test(pSuite, "policy_response 3", unittest_policy_response_3)  == NULL ||
        
        CU_add_test(pSuite, "policy_response 4", unittest_policy_response_4)  == NULL ||
        CU_add_test(pSuite, "policy_response 5", unittest_policy_response_5)  == NULL ||
        CU_add_test(pSuite, "policy_response 6", unittest_policy_response_6)  == NULL ||

        CU_add_test(pSuite, "Central_Bank_read_macro_indicators", unittest_Central_Bank_read_macro_indicators)  == NULL ||
        CU_add_test(pSuite, "Central_Bank_activate_policy_responses", unittest_Central_Bank_activate_policy_responses)  == NULL //||
    )
    {
        CU_cleanup_registry();
        return CU_get_error();
