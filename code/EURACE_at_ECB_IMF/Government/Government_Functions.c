...
int Government_install_human_capital_policy()
{
	...
	for(i=0; i<NEW_SKILL_DISTRIBUTION.size-1;i++)
	{
		while(actual_skill_distribution.array[i].id_list.size>NEW_SKILL_DISTRIBUTION.array[i].percentage*total_no_households)
		{
			...
			printf("Before: actual_skill_distribution.array[i].id_list.size %d ", actual_skill_distribution.array[i].id_list.size);			
			
			// Remove the first household (which has moved to the next skill group)
			remove_int(&actual_skill_distribution.array[i].id_list,0);

			printf("After: actual_skill_distribution.array[i].id_list.size %d \n ", actual_skill_distribution.array[i].id_list.size);			


			printf("Before: actual_skill_distribution.array[i+1].id_list.size %d ", actual_skill_distribution.array[i+1].id_list.size);	
			add_int(&actual_skill_distribution.array[i+1].id_list,id);
			printf("After: actual_skill_distribution.array[i+1].id_list.size %d \n ", actual_skill_distribution.array[i+1].id_list.size);	
...
{   
    ...
    START_TAX_PAYMENT_MESSAGE_LOOP
        ...
        log_inflow("government", "payment_account", "net_worth", "taxes", ID, tax_payment_message->tax_payment);
    ...
    START_UNEMPLOYMENT_BENEFIT_RESTITUTION_MESSAGE_LOOP
        ...
        log_inflow("government", "payment_account", "net_worth", "restitution", ID, unemployment_benefit_restitution_message->restitution_payment);
...
int Government_read_unemployment_benefit_notifications()
{
    ...
    START_UNEMPLOYMENT_NOTIFICATION_MESSAGE_LOOP
        ...
        log_outflow("government", "payment_account", "net_worth", "benefits", ID, unemployment_notification_message->unemployment_payment);
...
int Government_read_subsidy_notifications()
{
    ...
    //Filter: m.gov_id==a.id
        if(hh_subsidy_notification_message->gov_id==ID)
        {
           ...
           log_outflow("government", "payment_account", "net_worth", "subsidies", ID, hh_subsidy_notification_message->subsidy_payment);
 ...
 */
int Government_read_ecb_dividends()
{
    START_ECB_DIVIDEND_MESSAGE_LOOP
        ...
        MONTHLY_ECB_DIVIDEND+=ecb_dividend_message->dividend_per_gov;

        log_inflow("government", "payment_account", "net_worth", "cb_dividends", ID, ecb_dividend_message->dividend_per_gov);
...
{ 
  if(DAY%MONTH == 0)
  {
    ...
    log_stock("government", "payment_account", ID, PAYMENT_ACCOUNT);
    log_stock("government", "debt", ID, TOTAL_DEBT);
    log_stock("government", "net_worth", ID, PAYMENT_ACCOUNT-TOTAL_DEBT);
...
int Government_monthly_budget_accounting()
{
    ...
    YEARLY_ECB_DIVIDEND += MONTHLY_ECB_DIVIDEND;
    
    //Items that have already been added to the payment_account
        in = MONTHLY_TAX_REVENUES + MONTHLY_ECB_DIVIDEND;
        MONTHLY_INCOME = in;
    ...
    //Items that have already been subtracted from the payment_account
        ...
        if (PRINT_DEBUG_GOV)
    		{
    			if( fabs(TOTAL_DEBT + PAYMENT_ACCOUNT)>1e-6 )
                	printf("\n ERROR in Government: Total debt %2.5f is not equal to payment account %2.5f\n\n", TOTAL_DEBT, PAYMENT_ACCOUNT);
    		}
        #endif 
                
        //Monetary policy rule: decide on fraction of deficit to be financed by bonds/fiaT money
        ...
        if (PAYMENT_ACCOUNT<0.0)
        {
            ...
            TOTAL_DEBT += TOTAL_MONEY_FINANCING;
            ...
            log_inflow("government", "payment_account", "debt", "new_cb_debt", ID, TOTAL_MONEY_FINANCING);
        }
        else
        {
            //Repay debt from positive payment account
            double delta = min(PAYMENT_ACCOUNT, TOTAL_DEBT);
            TOTAL_DEBT -= delta;
            PAYMENT_ACCOUNT  -= delta;

            log_outflow("government", "payment_account", "debt", "repaid_cb_debt", ID, delta);

            //Let CB know about destruction of outside money
            add_request_fiat_money_message(-delta);
        }

            assert(TOTAL_DEBT>-EPSILON);
            assert(PAYMENT_ACCOUNT>-EPSILON);
        ...
        NET_WORTH_TO_GDP = (PAYMENT_ACCOUNT-TOTAL_DEBT) / (MONTHLY_GDP * 12);
...
int Government_yearly_budget_accounting()
{
   ...
   // GOV_INTEREST_RATE = 0.05;
    ...
    //Items that have already been added to the payment_account
        in = YEARLY_TAX_REVENUES + YEARLY_ECB_DIVIDEND;
        YEARLY_INCOME = in;
...
{   
    ...
    // Adjust tax rate
    double tax_base = (YEARLY_INCOME-YEARLY_ECB_DIVIDEND)/TAX_RATE_HH_LABOUR;

    if (tax_base>0)
        TAX_RATE_HH_LABOUR = (1-TAX_RATE_ADAPTION_SPEED)*TAX_RATE_HH_LABOUR + TAX_RATE_ADAPTION_SPEED*(max(0.0,min(1.0, (YEARLY_EXPENDITURE-YEARLY_ECB_DIVIDEND) / tax_base)));
