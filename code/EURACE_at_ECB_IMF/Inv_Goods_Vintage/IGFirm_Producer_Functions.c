...
int IGFirm_innovation_process()
{
	...
	/*If the innovation progress is random and does not have a predetermined shape*/
	if(INNOVATION_BY_RANDOM==1)
	{
        ...
        } //TRANSITION_PHASE_INNOV
    } //INNOVATION_BY_RANDOM
	else//Innovation progress has a predetermined shape: 4 cases: no, low, medium and high tech progress
	{
		int innovate=0;

		//Policy experiment:
		if(DAY > TRANSITION_PHASE_INNOV)
			FLAME_environment_variable_strength_of_tech_progress = COUNTERFACTUAL_STRENGTH_OF_TECH_PROGRESS;

		
		switch(STRENGTH_OF_TECH_PROGRESS)
		{
			case 1 :
                    if(DAY==INNOVATION_SCHEME_LOW_PROGRESS.array[0])
                    {
                        innovate=1;
                        remove_int(&INNOVATION_SCHEME_LOW_PROGRESS,0);
                    }
                    break;

			case 2 :
                    if(DAY==INNOVATION_SCHEME_MEDIUM_PROGRESS.array[0])
                    {
                        innovate=1;
                        remove_int(&INNOVATION_SCHEME_MEDIUM_PROGRESS,0);
                    }
                    break;

			case 3 :
                    if(DAY==INNOVATION_SCHEME_HIGH_PROGRESS.array[0])
                    {
                        innovate=1;
                        remove_int(&INNOVATION_SCHEME_HIGH_PROGRESS,0);
                    }
                    break;
		...
		if(innovate)
		{
			...
			CAPITAL_GOOD_PRICE = CAPITAL_GOOD_PRICE*(1+IGFIRM_EXOGENOUS_PRODUCTIVITY_PROGRESS);

			if(IG_GOOD_LIMITED_OFFER && VINTAGES.size >= MAX_OFFER_IG_GOOD)
			{
				//remove entry (oldest vintage)
				remove_vintage(&VINTAGES,0);
			}				
        ...
        {        
            switch(STRENGTH_OF_TECH_PROGRESS)
            {
                case 1 : printf("\t\t\t\tDAY %d\tINNOVATION_SCHEME_LOW_PROGRESS.array[0] %d\n",DAY, INNOVATION_SCHEME_LOW_PROGRESS.array[0]);
                         break;
                
                case 2 : printf("\t\t\t\tDAY %d\tINNOVATION_SCHEME_MEDIUM_PROGRESS.array[0] %d\n",DAY, INNOVATION_SCHEME_MEDIUM_PROGRESS.array[0]);
                         break;
                
                case 3 : printf("\t\t\t\tDAY %d\tINNOVATION_SCHEME_HIGH_PROGRESS.array[0] %d\n",DAY, INNOVATION_SCHEME_HIGH_PROGRESS.array[0]);
                         break;
...
int IGFirm_set_price_send_info()
{
	...
	//printf("ECONOMY_WIDE_SPECIFIC_SKILLS_IN_FIRMS %f\n",ECONOMY_WIDE_SPECIFIC_SKILLS_IN_FIRMS);
		...
		//Compute new price: Price is a linear combination of productivity gains and costs
		if(DAY>1)
			VINTAGES.array[i].price = (1-LINEAR_COMBINATION_PRICING_IG_GOOD) 
										* price_worst_vintage_last_month 
										* VINTAGES.array[i].discounted_productivity / discounted_productivity_worst_vintage_last_month 
									+ LINEAR_COMBINATION_PRICING_IG_GOOD 
										* UNIT_COSTS;
		
		if (VINTAGES.array[i].price < 0.0)
		{
			fprintf(stderr, "\n\nVINTAGES.array[%d].price %f", i, VINTAGES.array[i].price);
			fprintf(stderr, "\n\n(1-LINEAR_COMBINATION_PRICING_IG_GOOD) %f", (1-LINEAR_COMBINATION_PRICING_IG_GOOD));
			fprintf(stderr, "\n\nprice_worst_vintage_last_month %f", price_worst_vintage_last_month);
			fprintf(stderr, "\n\nVINTAGES.array[i].discounted_productivity %f", VINTAGES.array[i].discounted_productivity);
			fprintf(stderr, "\n\ndiscounted_productivity_worst_vintage_last_month %f", discounted_productivity_worst_vintage_last_month );
			fprintf(stderr, "\n\nLINEAR_COMBINATION_PRICING_IG_GOOD %f", LINEAR_COMBINATION_PRICING_IG_GOOD);
			fprintf(stderr, "\n\nUNIT_COSTS %f", UNIT_COSTS);
		
			fprintf(stderr, "\n\n IT %d ABORT! IGFirm sets negative price for vintage! Now exiting", DAY);
			
			assert(VINTAGES.array[i].price > 0.0);
		}
...
int IGFirm_calc_revenue()
{
	...
	START_PAY_CAPITAL_GOODS_MESSAGE_LOOP
		...
		log_inflow("ig_firm", "payment_account", "net_worth", "ig_revenue", ID, pay_capital_goods_message->capital_bill);
