...
int Household_receive_wage()
{
    ...
    START_WAGE_PAYMENT_MESSAGE_LOOP
        ...
        MONTHLY_BOND_INTEREST_INCOME + OP_PROFIT;

        /*Add wage to payment account.*/
        PAYMENT_ACCOUNT += wage_payment_message->payment;
        log_inflow("household", "payment_account", "net_worth", "wages", ID, wage_payment_message->payment);
    ...
    //Sanity Check:
    enforce_hh_valid_payment_account("Household_receive_wage");
...
int Household_UNEMPLOYED_read_job_vacancies_and_send_applications()
{
	...
	double rand2 = random_unif();
	double p2 = (1/(1+exp(-LAZY_UNEMPLOYEDS_STEEPNESS*(UNEMPLOYED_DURATION - LAZY_UNEMPLOYEDS_MIDPOINT))))*p/100;
...
int Household_send_unemployment_benefit_notification()
{
    ...
    //if unemployment benefit is larger than the min subsistene level:
        if(LAST_LABOUR_INCOME*UNEMPLOYMENT_BENEFIT_PCT > MINIMUM_SUBSISTENCE_LEVEL )
        {       
            UNEMPLOYMENT_PAYMENT = LAST_LABOUR_INCOME*UNEMPLOYMENT_BENEFIT_PCT;  
        }
        else    
        {
            //if unemployment benefit is below the min subsistene level
            UNEMPLOYMENT_PAYMENT =  MINIMUM_SUBSISTENCE_LEVEL; 
        }
    ...
    log_inflow("household", "payment_account", "net_worth", "benefits", ID, UNEMPLOYMENT_PAYMENT);

    //Sanity Check:
    if (PAYMENT_ACCOUNT < -0.01)
        fprintf(stderr, "\n\nIT %d ID %d Household_send_unemployment_benefit_notification: PAYMENT_ACCOUNT < -0.01, PAYMENT_ACCOUNT=%.2f", DAY, ID, PAYMENT_ACCOUNT);
  ...
  //  printf("\n UNEMPLOYMENT_BENEFIT_PCT: %f LAST_LABOUR_INCOME: %f",UNEMPLOYMENT_BENEFIT_PCT,LAST_LABOUR_INCOME);
    ...
    TOTAL_INCOME=  UNEMPLOYMENT_PAYMENT + CUM_TOTAL_DIVIDENDS + MONTHLY_BOND_INTEREST_INCOME + OP_PROFIT;
...
int Household_send_subsidy_notification()
{
    ...
    //Sanity Check:
    if (PAYMENT_ACCOUNT < -0.01)
        fprintf(stderr, "\n\nIT %d ID %d Household_send_subsidy_notification: PAYMENT_ACCOUNT < -0.01, PAYMENT_ACCOUNT=%.2f", DAY, ID, PAYMENT_ACCOUNT);
...
int Household_send_transfer_notification()
{
    ...
    //Sanity Check:
    if (PAYMENT_ACCOUNT < -0.01)
        fprintf(stderr, "\n\nIT %d ID %d Household_send_transfer_notification: PAYMENT_ACCOUNT < -0.01, PAYMENT_ACCOUNT=%.2f", DAY, ID, PAYMENT_ACCOUNT);
...
int Household_send_tax_payment()
{
   ...
   // #endif
    ...
    PAYMENT_ACCOUNT -= restitution_payment;
	log_outflow("household", "payment_account","net_worth", "restitution", ID, restitution_payment);


    /*Compute the total taxes*/
    TAX_PAYMENT = (CUM_TOTAL_DIVIDENDS + max(0, OP_PROFIT))*TAX_RATE_HH_CAPITAL + (WAGE+UNEMPLOYMENT_PAYMENT)*TAX_RATE_HH_LABOUR;

    // DIRTY FIX to avoid negative payment accounts... (TODO)
    if(TAX_PAYMENT > PAYMENT_ACCOUNT)
    {
    	printf("WARNING: Household unable to make full tax payment! ID: %d, tax_payment: %f, payment_account: %f\n", ID, TAX_PAYMENT, PAYMENT_ACCOUNT);
    	TAX_PAYMENT = PAYMENT_ACCOUNT;
    }
    ...
    log_outflow("household", "payment_account","net_worth", "taxes", ID, TAX_PAYMENT);

    /*Setting the counter of monthly dividends = 0*/
    CUM_TOTAL_DIVIDENDS=0;

    //Sanity Check:
    enforce_hh_valid_payment_account("Household_send_tax_payment");
