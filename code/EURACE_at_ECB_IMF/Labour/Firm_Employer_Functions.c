...
int Firm_send_random_redundancies()
{
	...
	{  
		...
		if (FIX_EMPLOYEE_BUDGET)
			REMAINING_BUDGET_FOR_HIRING+=EMPLOYEES.array[j].wage;
...
int Firm_read_job_applications_send_job_offer_or_rejection()
{
	...
	// Copy remainign budget to send out offers
	// REMAINING_BUDGET_FOR_HIRING is updated, wehen job accepted
	...
	else
	{
		...
		for(i = 0; i< VACANCIES; i++)
		{
	        ...
	        assert(denominator_logit<DBL_MAX);
			...
			for(j = 0; j < logit_applications_list.size; j++)
			{
				...
				if(random_number<sum_of_logits)
				{
					...
					switch(logit_applications_list.array[j].general_skill)
					{
						...
						case 1:
							if (budget_remaining >= WAGE_OFFER_FOR_SKILL_1)
							{
								add_job_offer_message(ID, 
						...
						case 2:
							if (budget_remaining >= WAGE_OFFER_FOR_SKILL_2)
							{
								add_job_offer_message(ID, 
								logit_applications_list.array[j].worker_id,
								REGION_ID, 
						...
						case 3:
							if (budget_remaining >= WAGE_OFFER_FOR_SKILL_3)
							{
								add_job_offer_message(ID, 
								logit_applications_list.array[j].worker_id,
								REGION_ID, 
						...
						case 4:
							if (budget_remaining >= WAGE_OFFER_FOR_SKILL_4)
							{
								add_job_offer_message(ID, 
								logit_applications_list.array[j].worker_id,
								REGION_ID, 
						...
						case 5:
							if (budget_remaining >= WAGE_OFFER_FOR_SKILL_5)
							{
								add_job_offer_message(ID, 
								logit_applications_list.array[j].worker_id,
								REGION_ID, 
					...
					// Also remove him, if he is too expensive
...
{   
	...
	{
		printf("WARNING: remaining budget <0 after labour market 1 (ID: %d, budget: %f)\n",ID, REMAINING_BUDGET_FOR_HIRING);
	}
...
{   
	...
	if(REMAINING_BUDGET_FOR_HIRING < -EPSILON)
	{
		printf("WARNING: remaining budget <0 after labour market 2 (ID: %d, budget: %f)\n",ID, REMAINING_BUDGET_FOR_HIRING);
	}
