...
void unittest_Firm_set_bankruptcy_illiquidity()
{
    ...
    rc = MB_Create(&b_firm_bankruptcy_illiquidity, sizeof(m_firm_bankruptcy_illiquidity));
            //#ifdef ERRCHECK
            if (rc != MB_SUCCESS)
            {
               fprintf(stderr, "ERROR: Could not create 'firm_bankruptcy_illiquidity' board\n");
               switch(rc) {
                   case MB_ERR_INVALID:
    ...
    rc = MB_Iterator_Create(b_firm_bankruptcy_illiquidity, &i_firm_bankruptcy_illiquidity);
            
    if (rc != MB_SUCCESS)
            {
               fprintf(stderr, "ERROR: Could not create Iterator for 'firm_bankruptcy_illiquidity'\n");
               switch(rc) {
                   case MB_ERR_INVALID:
                       fprintf(stderr, "\t reason: 'firm_bankruptcy_illiquidity' board is invalid\n");
                       break;
                   case MB_ERR_LOCKED:
                       fprintf(stderr, "\t reason: 'firm_bankruptcy_illiquidity' board is locked\n");
                       break;
                   case MB_ERR_MEMALLOC:
                       fprintf(stderr, "\t reason: out of memory\n");
                       break;
    ...
    CU_ASSERT_DOUBLE_EQUAL(TOTAL_VALUE_LOCAL_INVENTORY, 10.0, 1e-3);
	...
	printf("\n\t Found msg\n");
	printf("\n\t firm_bankruptcy_illiquidity_message->firm_id=%d\n", firm_bankruptcy_illiquidity_message->firm_id);
	printf("\n\t firm_bankruptcy_illiquidity_message->value=%2.3f\n", firm_bankruptcy_illiquidity_message->value);
	
         CU_ASSERT_EQUAL(firm_bankruptcy_illiquidity_message->firm_id, 1);
	 CU_ASSERT_DOUBLE_EQUAL(firm_bankruptcy_illiquidity_message->value, 10.0, 1e-3);
...
void unittest_Firm_set_bankruptcy_insolvency()
{
    ...
    rc = MB_Create(&b_firm_bankruptcy_insolvency, sizeof(m_firm_bankruptcy_insolvency));
            #ifdef ERRCHECK
            if (rc != MB_SUCCESS)
            {
               fprintf(stderr, "ERROR: Could not create 'firm_bankruptcy_insolvency' board\n");
               switch(rc) {
                   case MB_ERR_INVALID:
                       fprintf(stderr, "\t reason: Invalid message size\n");
                       break;
    ...
    /***** Message: Adding message iterators ***************************************/
    rc = MB_Iterator_Create(b_firm_bankruptcy_insolvency, &i_firm_bankruptcy_insolvency);
            
    if (rc != MB_SUCCESS)
            {
               fprintf(stderr, "ERROR: Could not create Iterator for 'firm_bankruptcy_insolvency'\n");
               switch(rc) {
                   case MB_ERR_INVALID:
                       fprintf(stderr, "\t reason: 'firm_bankruptcy_insolvency' board is invalid\n");
                       break;
                   case MB_ERR_LOCKED:
                       fprintf(stderr, "\t reason: 'firm_bankruptcy_insolvency' board is locked\n");
                       break;
                   case MB_ERR_MEMALLOC:
                       fprintf(stderr, "\t reason: out of memory\n");
                       break;
    ...
    START_FIRM_BANKRUPTCY_INSOLVENCY_MESSAGE_LOOP
    	printf("\n\t Found msg\n");
	printf("\n\t firm_bankruptcy_insolvency_message->firm_id=%d\n", firm_bankruptcy_insolvency_message->firm_id);
	printf("\n\t firm_bankruptcy_insolvency_message->value=%2.3f\n", firm_bankruptcy_insolvency_message->value);

         CU_ASSERT_EQUAL(firm_bankruptcy_insolvency_message->firm_id, 1);
	 CU_ASSERT_DOUBLE_EQUAL(firm_bankruptcy_insolvency_message->value, -10.0, 1e-3);
...
/*
 * \fn: void unittest_Firm_check_recapitalization()
 * \brief: Unit test for: Firm_check_recapitalization.
 * Status: NOT Tested
 */
void unittest_Firm_check_recapitalization()
{
    int rc;
    ...
    /************* Setting environment variables **************/
    //FLAME_environment_variable_day = 1;
    //FLAME_environment_variable_const_init_transient_period = 100;
    ...
    rc = MB_Create(&b_firm_recapitalization, sizeof(m_firm_recapitalization));
            //#ifdef ERRCHECK
            if (rc != MB_SUCCESS)
            {
               fprintf(stderr, "ERROR: Could not create 'firm_recapitalization' board\n");
               switch(rc) {
                   case MB_ERR_INVALID:
                       fprintf(stderr, "\t reason: Invalid message size\n");
                       break;
    ...
    /***** Messages: pre-conditions **********************************/
	add_firm_recapitalization_message(1, 100.0);
	
    /***** Message: Adding message iterators ***************************************/
    rc = MB_Iterator_Create(b_firm_recapitalization, &i_firm_recapitalization);
            
    if (rc != MB_SUCCESS)
            {
               fprintf(stderr, "ERROR: Could not create Iterator for 'firm_recapitalization'\n");
               switch(rc) {
                   case MB_ERR_INVALID:
                       fprintf(stderr, "\t reason: 'firm_recapitalization' board is invalid\n");
                       break;
                   case MB_ERR_LOCKED:
                       fprintf(stderr, "\t reason: 'firm_recapitalization' board is locked\n");
                       break;
                   case MB_ERR_MEMALLOC:
                       fprintf(stderr, "\t reason: out of memory\n");
                       break;
    ...
    /***** Function evaluation ***************************************/
    Firm_check_recapitalization();

    /***** Variables: Memory post-conditions *****/
    
    CU_ASSERT_DOUBLE_EQUAL(PAYMENT_ACCOUNT, 120.0, 1e-3);
    CU_ASSERT_DOUBLE_EQUAL(EQUITY, 90.0, 1e-3);
...
*/
    ...
    /***** Messages: initialize message boards **********************************/
    rc = MB_Create(&b_firm_bankruptcy, sizeof(m_firm_bankruptcy));
            #ifdef ERRCHECK
            if (rc != MB_SUCCESS)
            {
    ...
    //19.10.2016: Function no longer exists?
    //Firm_bankruptcy_insolvency_procedure();    
    
    /***** Message: Adding message iterators ***************************************/
    //add_firm_bankruptcy_message(bank_id, bad_debt, credit_refunded, writeoff_exposure);

    rc = MB_Iterator_Create(b_firm_bankruptcy, &i_firm_bankruptcy);
    ...
    START_FIRM_BANKRUPTCY_MESSAGE_LOOP
        ...
        CU_ASSERT_EQUAL(firm_bankruptcy_message->bank_id, 1);
        CU_ASSERT_DOUBLE_EQUAL(firm_bankruptcy_message->bad_debt, 70.0, 1e-3);     
        CU_ASSERT_DOUBLE_EQUAL(firm_bankruptcy_message->credit_refunded, 10.0, 1e-3);
        CU_ASSERT_DOUBLE_EQUAL(firm_bankruptcy_message->writeoff_exposure, 150.0, 1e-3);
...
*/
    ...
    /***** Message: Adding message iterators ***************************************/
    //add_bankruptcy_message(bank_id, bad_debt, credit_refunded, writeoff_exposure);
...
void unittest_Firm_read_stock_transactions()
{
    ...
    /***** Messages: pre-conditions **********************************/
    //OLD code: add_order_status_message(trader_id, asset_id, price, quantity)
    //NEW code: add_order_status_message(trader_id, value, quantity)
    
    add_order_status_message(ID, 1.0, -100);
