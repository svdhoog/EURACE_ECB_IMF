...
{		
	...
	{
		add_debt_item(&temp_loans, 0,0,LOANS.array[i].loan_value,LOANS.array[i].interest_rate,LOANS.array[i].installment_amount
			,0,LOANS.array[i].nr_periods_before_repayment);
	}
	...
	for(Y=0; Y <year-1;Y++)
	{
		...
		for(t=0;t<12;t++)
		{
			...
			if(debt_needed>1e-5)
			{
				add_debt_item(&temp_loans, 0,0,debt_needed,LAST_INTEREST_RATE_FOR_LOANS,debt_needed/CONST_INSTALLMENT_PERIODS,0,CONST_INSTALLMENT_PERIODS);
			}
...
int Firm_set_market_research_data_pricing()
{
	...
	//2. Quality Index: protect against CURRENT_QUALITY_INDEX==0.0
	if(VARIABLES_LAST_YEAR.array[0].quality_index>1e-5 && CURRENT_QUALITY_INDEX > 1e-3)
	{
	...
	assert__(VARIABLES_CHANGE.quality_index > -1.0)
	{
		fprintf(stderr, "\nIT %d ID %d", DAY, ID);
        fprintf(stderr, "\n\t CURRENT_QUALITY_INDEX %f", CURRENT_QUALITY_INDEX);
        fprintf(stderr, "\n\t VARIABLES_LAST_YEAR.array[0].quality_index %f", VARIABLES_LAST_YEAR.array[0].quality_index);
        fprintf(stderr, "\n\t CURRENT_QUALITY_INDEX - VARIABLES_LAST_YEAR.array[0].quality_index %f", CURRENT_QUALITY_INDEX - VARIABLES_LAST_YEAR.array[0].quality_index);
        fprintf(stderr, "\n\t (CURRENT_QUALITY_INDEX - VARIABLES_LAST_YEAR.array[0].quality_index)/VARIABLES_LAST_YEAR.array[0].quality_index %f", (CURRENT_QUALITY_INDEX - VARIABLES_LAST_YEAR.array[0].quality_index)/VARIABLES_LAST_YEAR.array[0].quality_index);
        fprintf(stderr, "\n\t VARIABLES_CHANGE.quality_index %f", VARIABLES_CHANGE.quality_index);
        fprintf(stderr, "\n");
...
int Firm_send_questionnaire()
{
	...
	{
		assert(VARIABLES_CHANGE.quality_index > -1.0);
...
int Firm_analyze_questionnaire()
{
	...
	//Reset old demand functions
	reset_dt_demand_function_array(&DEMAND_FUNCTIONS_PRODUCT_INNOVATION);
...
int	Firm_set_price()
{
	...
	// Reset array PROSPECTIVE_VARIABLES
	reset_dt_prospective_variables_array(&PROSPECTIVE_VARIABLES);

	// Reset vector of optimal price choices for the next years until the completation of the R & D project	
	reset_double_array(&EXPECTED_PRICE_CHOICES);
	...
	for(i=0;i<LOANS.size;i++)
	{
		add_debt_item(&temp_loans_at_end_of_year, 0,0,LOANS.array[i].loan_value,LOANS.array[i].interest_rate,LOANS.array[i].installment_amount
			,0,LOANS.array[i].nr_periods_before_repayment);
	}
	...
	for(Y=0;Y<MARKET_RESEARCH_DATA_PRICING.array[0].estimated_values.size-1;Y++)
	{
		...
		for(i=0;i<MARKET_RESEARCH_DATA_PRICING.size;i++)
		{
			...
			{
				add_debt_item(&temp_loans, 0,0,temp_loans_at_end_of_year.array[j].loan_value,temp_loans_at_end_of_year.array[j].interest_rate,temp_loans_at_end_of_year.array[j].installment_amount
					,0,temp_loans_at_end_of_year.array[j].nr_periods_before_repayment);
			}
		 ...
		 printf( "Price: %f   sum_earnings_discounted  %f    sum_earnings %f \n",MARKET_RESEARCH_DATA_PRICING.array[i].price, sum_earnings_discounted ,sum_earnings);
			...
			if(i==0 || temp_highest_earnings < MARKET_RESEARCH_DATA_PRICING.array[i].estimated_values.array[Y].estimated_earnings)
			{
				...
				if(Y!=MARKET_RESEARCH_DATA_PRICING.array[0].estimated_values.size-1)
				{
					...
					for(j=0;j<temp_loans.size;j++)
					{
						add_debt_item(&PROSPECTIVE_VARIABLES.array[Y].loans, 0,0,temp_loans.array[j].loan_value
						,temp_loans.array[j].interest_rate,temp_loans.array[j].installment_amount
							,0,temp_loans.array[j].nr_periods_before_repayment);
					}
		...
		//Pass these variables of the optimal price strategy to the next considered year 
		if(Y<MARKET_RESEARCH_DATA_PRICING.array[0].estimated_values.size-1)
		{

			reset_adt_capital_stock_vintages_array(&capital_stock_at_end_of_year);
			...
			reset_financing_capital_array(&temp_capital_financing_at_end_of_year);
			...
			reset_debt_item_array(&temp_loans_at_end_of_year);
				
			for(i=0;i<PROSPECTIVE_VARIABLES.array[Y].loans.size;i++)
			{
				add_debt_item(&temp_loans_at_end_of_year, 0,0,PROSPECTIVE_VARIABLES.array[Y].loans.array[i].loan_value
					,PROSPECTIVE_VARIABLES.array[Y].loans.array[i].interest_rate,PROSPECTIVE_VARIABLES.array[Y].loans.array[i].installment_amount
					,0,PROSPECTIVE_VARIABLES.array[Y].loans.array[i].nr_periods_before_repayment);
...
int Firm_decide_product_innovation()
{
	...
	reset_dt_best_price_strategies_array(&BEST_PRICING_STRATEGIES);
		...
		//	init_debt_item_array(&temp_loans_at_end_of_year);
			...
			for(j=0; j <MARKET_RESEARCH_DATA_PRODUCT_INNOVATION.array[i].market_research_price.size;j++)
			{
				...
				init_debt_item_array(&temp_loans);
					...
					{
						add_debt_item(&temp_loans, 0,0,PROSPECTIVE_VARIABLES.array[PROSPECTIVE_VARIABLES.size-1].loans.array[k].loan_value,PROSPECTIVE_VARIABLES.array[PROSPECTIVE_VARIABLES.size-1].loans.array[k].interest_rate,
							PROSPECTIVE_VARIABLES.array[PROSPECTIVE_VARIABLES.size-1].loans.array[k].installment_amount
							,0,	PROSPECTIVE_VARIABLES.array[PROSPECTIVE_VARIABLES.size-1].loans.array[k].nr_periods_before_repayment);
					}
				...
				for(t=0;t<12;t++)
				{
					...
					if(debt_needed>1e-5)
					{
					add_debt_item(&temp_loans, 0,0,debt_needed,LAST_INTEREST_RATE_FOR_LOANS,debt_needed/CONST_INSTALLMENT_PERIODS,
						0,CONST_INSTALLMENT_PERIODS);
					}
					payment_account+= debt_needed;
...
BEST_PRICING_ABANDON_STRATEGY.sum_expected_earnings_discounted ,BEST_PRICING_ABANDON_STRATEGY.sum_earnings);
	...
	// Reset lattice
	reset_dt_value_lattice_years_array(&VALUE_LATTICE);
...
int Firm_clean_up_arrays()
{
	...
	reset_dt_market_research_qualities_array(&MARKET_RESEARCH_DATA_PRODUCT_INNOVATION);

	reset_dt_market_research_price_array(&MARKET_RESEARCH_DATA_PRICING);
